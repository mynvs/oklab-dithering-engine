...
system(f'cd output && del *.png')

if use_all_palettes:
	for filename in listdir('palettes'):
		print(f'bin\\dither -o output\\{filename[:-4]} -c {filename[:-4]}')
		system(f'bin\\dither -o output\\{filename[:-4]} -c {filename[:-4]}')
else:
	for filename in palette_list:
		print(f'bin\\dither -o output\\{filename} -c {filename}')
		system(f'bin\\dither -o output\\{filename} -c {filename}')
