...
struct dither_settings {
    algorithm stalg;
    double l_offset;
    double l_scale; // -inf to +inf
    double hue_phase;
    double hue_scale;
    double a_offset;
    double b_offset;
    double dither_intensity;
};

struct palette_info {
    int num_colors;
    std::shared_ptr<double[]> L;
...
void srgb_oklab(unsigned char rgb[3], double lab[3]) {
    ...
    // linear srgb -> oklab
    const double d = powf(0.4122214708 * a + 0.5363325363 * b + 0.0514459929 * c, 0.3333333333333333);
    const double e = powf(0.2119034982 * a + 0.6806995451 * b + 0.1073969566 * c, 0.3333333333333333);
    const double f = powf(0.0883024619 * a + 0.2817188376 * b + 0.6299787005 * c, 0.3333333333333333);
...
void load_palette(const std::string& palette_ppm_filename, palette_info& palette) {
    ...
    palette.L = std::shared_ptr<double[]>(new double[palette.num_colors]);
    palette.a = std::shared_ptr<double[]>(new double[palette.num_colors]);
    palette.b = std::shared_ptr<double[]>(new double[palette.num_colors]);

    memset(palette.L.get(), 0, sizeof(double) * num_colors);
    memset(palette.a.get(), 0, sizeof(double) * num_colors);
    memset(palette.b.get(), 0, sizeof(double) * num_colors);
...
void dither(const dither_settings& settings,
    ...
    const double hue_phase_cos = std::cos(settings.hue_phase * TAU);
    const double hue_phase_sin = std::sin(settings.hue_phase * TAU);
    const double l_scale = settings.l_scale;
    const double hue_phase_cos_scale = hue_phase_cos * settings.hue_scale;
    const double hue_phase_sin_scale = hue_phase_sin * settings.hue_scale;
    const double l_offset = settings.l_offset;
    const double a_offset = settings.a_offset;
    const double b_offset = settings.b_offset;
    ...
    if (settings.stalg == errordiffuse) {
        const double negative_dither_intensity = -settings.dither_intensity;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                int min_idx = -1;
                for (int i = 0; i < num_colors; i++) {
                    double L_diff = lab[0] - palette_L[i];
                    double a_diff = lab[1] - palette_a[i];
                    double b_diff = lab[2] - palette_b[i];
                    double dist = std::sqrt(L_diff * L_diff + a_diff * a_diff + b_diff * b_diff);
                    if (dist < min_dist) {
                        min_dist = dist;
                        min_idx = i;
                    }
                ...
                double threshold = (matrix[iy][ix] + 0.5) / (matrix_size * matrix_size);
                double l = lab[0];
                if (l <= threshold) {
                    l = palette_L[min_idx];
                }
